<template>
<main>
	<section class="section">
		<div class="container page-hire">
			<h2 class="title is-3">
				Hire me
			</h2>
			<div class="content">
				<div class="field">
					<h4 class="title">联系方式</h4>
					<ul>
						<li>手机：156 6651 0209</li>
						<li>Email：ning.leez@gmail.com</li>
						<li>QQ：2936300</li>
						<li>微信号：mr_madness</li>
					</ul>
				</div>

				<div class="field">
					<h4 class="title">个人信息</h4>
					<ul>
						<li>栗宁/男/1983</li>
						<li>工作经验：web开发6年+，php开发2年</li>
						<li>期望职位：web前端开发，全栈开发</li>
						<li>期望薪资：税前月薪14k~17k，特别喜欢的公司可例外</li>
					</ul>
				</div>
				
				<div class="field">
					<h4 class="title">工作经历</h4>
				</div>

				<div class="field">
					<h5 class="title">淘唛(天津)网络科技有限公司 (2018年10月~至今)</h5>
					<h7>淘唛商城 taomaisc.com</h7>
					<ol>
						<li>服务器日常维护；</li>
						<li>商城专题策划；</li>
						<li>会员信心统筹收集；</li>
						<li>公众号维护；</li>
					</ol>
				</div>

				<div class="field">
					<h5 class="title">深圳链通网络科技有限公司 （ 2018年3月 ~ 2018年7月 ）</h5>
				</div>
				<div class="field">
					<h7>乐享链 lscworld.net</h7>
					<ol>
						<li>乐享区块链平台vi设计；</li>
						<li>乐享区块链平台PC端和wap端布局重新设计开发及后续优化；</li>
						<li>基于Hybrid App开发乐享链安卓端手机APP</li>
					</ol>
				</div>

				<div class="field">
					<h5>聊城康莱德健康产业有限公司 （ 2016年10月 ~ 2017年9月 ）</h5>
				</div>
				<div class="field">
					<h7>乐众挖金网</h7>
					<ol>
						<li>乐众挖金网PC端和wap端布局重新设计开发及后续优化；</li>
						<li>乐众挖金网部分特定需求交易模块的开发工作；</li>
						<li>乐众挖金网客服系统维护及功能优化</li>
					</ol>
				</div>

				<div class="field">
					<h5>山东高唐汇鑫网络科技服务中心 （ 2012年9月 ~ 2016年7月 ）</h5>
					<ol>
						<li>负责淘宝卖家业务培训；</li>
						<li>淘宝店铺装修业务，制图及sdk商业模板制作；</li>
						<li>discuz、wordpress模板制作；</li>
						<li>上海赢品商贸外包项目制作及维护：潮妈帮品牌论坛积分商城界面设计开发及后续优化；宝贝购品牌商城PC端及APP端界面设计、海外购品牌界面设计、微信活动页面设计及开发</li>
						<li>其他一些网站外包工作</li>
					</ol>
				</div>

				<div class="field">
					<h4>技能清单</h4>
					<ul>
						<li>Web开发：PHP</li>
						<li>Web框架或程序：Lavarel/Wordpress/Discuz</li>
						<li>前端框架：JQuery/Bootstrap/Semantic UI/Bulma/Framework7</li>
						<li>前端工具：Vue/WebPack/Npm</li>
						<li>开发工具：VSCode/Hbuilder/Sublime text/Notepad++/Postman/Git</li>
						<li>设计工具：AI/Fireworks</li>
						<li>数据库相关：MySQL</li>
						<li>服务器相关：CentOS基础配置及日常维护/LNMP安装调试</li>
						<li>使用语言：普通话/英语(阅读技术文档)</li>
					</ul>
				</div>

				<div class="field">
					<h4 class="title">个人网站</h4>
					<p><a href="https://mabuzki.com">https://mabuzki.com</a></p>
				</div>

				<hr>

				<div class="field">
					<h4 class="title">致谢</h4>
					<p>感谢您花时间阅读我的简历，期待能有机会和您共事。</p>
				</div>

			</div>
		</div>
	</section>
</main>
</template>

<script>
export default {
	name: 'hire'
}
</script>
